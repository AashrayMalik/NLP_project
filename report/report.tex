...
\begin{abstract}
This report describes an end-to-end NLP system built over a large-scale Reddit corpus scraped from the \texttt{r/jobs} subreddit. The pipeline spans five major stages: (1)~data collection and cleaning, (2)~BERTopic-based topic modelling with trending/persistent classification, (3)~zero-shot stance detection per topic, (4)~a hybrid retrieval-augmented generation (RAG) question-answering system combining dense vector retrieval, graph expansion, and deterministic SQL facts, and (5)~a multi-track evaluation covering baseline English QA, Hindi cross-lingual QA, extended retrieval diagnostics, and a causal-overclaim audit. The entire system is served through an interactive Streamlit dashboard. We report results across ROUGE-L, BERTScore, chrF, citation quality, refusal accuracy, paraphrase stability, and comment retrievability metrics.
\end{abstract}
...
\begin{enumerate}[leftmargin=*]
  \item \textbf{What are the dominant themes?} We apply BERTopic to discover latent topic clusters and classify each as persistent or trending.
...
\subsection{Trending vs.\ Persistent Topic Classification}
\label{sec:trending}
...
Classification uses a combination of absolute and relative thresholds:

\begin{itemize}[leftmargin=*]
  \item \textbf{Persistent}: appears in $\geq 70\%$ of months AND CV in the bottom third across topics AND low normalised slope. These are stable, ever-present discussion themes.
  \item \textbf{Trending}: has a spike (some month $> 2\times$ mean) OR appears in $< 50\%$ of months OR CV/slope in the top third. These are topics that emerged recently or surged.
  \item \textbf{Seasonal}: everything else --- moderate variation without extreme spikes.
\end{itemize}
...
\begin{enumerate}[leftmargin=*]
  ...
  \item \textbf{Classifier}: the \texttt{facebook/bart-large-mnli} model is used in a zero-shot classification pipeline with candidate labels:
  \begin{itemize}
    \item ``supporting the parent post''
    \item ``opposing the parent post''
    \item ``neutral or unclear''
  \end{itemize}
...
\begin{itemize}[leftmargin=*]
  ...
  \item Trend type (persistent/trending/seasonal)
...
Romanised and Reddit-slang inputs achieve the highest chrF scores (0.261, 0.262) because shared English vocabulary acts as character-level anchoring. Multi-hop reasoning (0.161) is the weakest category, consistent with the challenge of combining evidence from multiple retrieval results.


\subsection{Track 3: Extended Retrieval Diagnostics}
\label{sec:eval-diagnostics}

Beyond answer-level metrics, the diagnostics track evaluates \textbf{retrieval behaviour}:

\subsubsection{Query Type Accuracy}

Each diagnostic question specifies its expected query type (factual, trend, opinion, general). The metric is the fraction of questions where the system's automatic classification matches the expected type.

\subsubsection{Structure Preservation (Source-Type Hit Rate)}

Questions specify which evidence source types \textit{must} appear in the top-$k$ results (e.g., a factual question about corpus size should retrieve \texttt{sql\_fact} evidence). The hit rate measures whether the retriever surfaces the correct evidence categories.

\subsubsection{Comment Requirement Satisfaction}

For opinion and comment-heavy questions, a minimum number of comment-type evidence items is required. The satisfaction rate measures compliance.

\subsubsection{Paraphrase Stability}

Paraphrase groups contain semantically equivalent queries (e.g., ``What are the most discussed topics?'' and ``Which themes come up most often?''). For each pair within a group, the Jaccard similarity of their retrieved evidence ID sets is computed. High Jaccard indicates stable retrieval under paraphrasing.
...
\subsubsection{Comment Retrievability Probes}

A separate probe routine tests whether specific comments can be retrieved when used as query seeds:

\begin{enumerate}[leftmargin=*]
  \item \textbf{Stratified sampling}: 24 comments are selected across month $\times$ score-bucket strata (low/mid/high thirds by karma).
  \item \textbf{Probe query construction}: for each sampled comment, a synthetic query is built from its parent post title, topic label, and comment excerpt.
  \item \textbf{Metrics}: initial hit rate (comment appears in vector candidates), final hit rate (comment appears in reranked top-$k$), and mean final rank when hit.
\end{enumerate}
...
\subsection{Track 4: Causal Overclaim Audit}
...
Displays aggregate corpus statistics via KPI cards (total posts, comments, unique authors, average lengths and scores), monthly activity bar charts, flair distribution pie charts, engagement tier breakdowns, score distributions, and average length trends over time.

\subsection{Topic Analysis Page}

Presents discovered topics as interactive cards with display labels, keyword badges, corpus share percentages, and trend-type badges. A topic-vs-flair analysis chart identifies gaps where topics are not well-covered by existing Reddit flairs. Expandable details show representative posts and model-assigned labels.

\subsection{Trends Page}

Visualises trending vs.\ persistent classifications with monthly sparklines colour-coded by trend type (green for persistent, amber for trending, cyan for seasonal). Narrative summaries automatically describe the temporal dynamics of the corpus.

\subsection{Stance Detection Page}

Displays per-topic stance distributions with stacked bars (for/opposing/neutral) and representative argument quotes styled with colour-coded borders. Users can explore individual topic stances and their top arguments.
...
The application uses a custom dark-theme CSS system with:
\begin{itemize}[leftmargin=*]
  \item Inter font family (Google Fonts) for clean, modern typography.
  \item Purple accent palette (\texttt{\#8b5cf6}, \texttt{\#c4b5fd}) for interactive elements.
  \item Glassmorphism-inspired card components with subtle hover effects.
  \item Plotly charts configured with a transparent dark theme matching the application aesthetic.
...
\begin{enumerate}[leftmargin=*]
  \item A production-quality data pipeline with comprehensive cleaning and jargon normalisation.
